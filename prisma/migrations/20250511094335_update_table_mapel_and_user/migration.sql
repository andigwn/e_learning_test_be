/*
  ...
  - Added the required column `id_users` to the `mata_pelajaran` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `mata_pelajaran` ADD COLUMN `id_users` INTEGER NOT NULL;
...
-- AddForeignKey
ALTER TABLE `mata_pelajaran` ADD CONSTRAINT `mata_pelajaran_id_users_fkey` FOREIGN KEY (`id_users`) REFERENCES `users`(`id_users`) ON DELETE CASCADE ON UPDATE CASCADE;
